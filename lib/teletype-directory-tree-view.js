...
class TeletypeDirectoryTreeView {
    ...
    renderDir(tree, path) {
        ...
        for (const tmpDir of tree.dirs) {
            const tmp = this.renderDir(tmpDir, path);
        ...
        for (const tmpFile of tree.files) {
            const tmp = this.renderFile(tmpFile, path);
            content_container.appendChild(tmp);
        }
    ...
    renderFile(file, path) {
        ...
        span_file.dataset.name = file.name;
        span_file.dataset.path = path+file.name;
        span_file.addEventListener('click', (e) => {
            this.openFile(e);
        })
